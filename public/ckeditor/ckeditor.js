...
(function() {
	...
	(function() {
		...
		a.tools = {
			...
			normalizeCssText: function(f, g) {
				var h = [],
					i, j = a.tools.parseCssText(f, true, g);
	...
	d.event.prototype = {
		...
		getPageOffset: function() {
			var j = this;
			var g = j.getTarget().getDocument().$,
				h = j.$.pageX || j.$.clientX + (g.documentElement.scrollLeft || g.body.scrollLeft),
				i = j.$.pageY || j.$.clientY + (g.documentElement.scrollTop || g.body.scrollTop);
			return {
				x: h,
				y: i
			};
	...
	(function() {
		e.extend(h.prototype, {
				...
				if (c && (b.ie7Compat || b.ie6Compat)) return function(m) {
					...
					switch (m) {
						...
						case 'tabindex':
							...
							break;
						case 'checked':
							var o = q.$.attributes.getNamedItem(m),
								p = o.specified ? o.nodeValue : q.$.checked;
			...
			isVisible: function() {
				var o = this;
				var l = (o.$.offsetHeight || o.$.offsetWidth) && o.getComputedStyle('visibility') != 'hidden',
					m, n;
			...
			removeAttributes: function(l) {
				if (e.isArray(l))
					...
					for (var n in l) l.hasOwnProperty(n) && this.removeAttribute(n);
			},
			...
			getDocumentPosition: function(l) {
				...
				var m = 0,
					...
					o = G.getDocument(),
					p = o.getBody(),
					q = o.$.compatMode == 'BackCompat';
				if (document.documentElement.getBoundingClientRect) {
					var r = G.$.getBoundingClientRect(),
						s = o.$,
						t = s.documentElement,
						u = t.clientTop || p.$.clientTop || 0,
						v = t.clientLeft || p.$.clientLeft || 0,
						w = true;
					if (c) {
						var x = o.getDocumentElement().contains(G),
							y = o.getBody().contains(G);
						w = q && y || !q && x;
					}
					if (w) {
			...
			scrollIntoParent: function(l, m, n) {
				...
				var t = l.getWindow(),
					...
					A, B;
				A = {
					x: u.x - s(this, 'left') - v.x || 0,
					y: u.y - s(this, 'top') - v.y || 0
				};
				B = {
					x: u.x + x + s(this, 'right') - (v.x + z) || 0,
					y: u.y + w + s(this, 'bottom') - (v.y + y) || 0
				};
			...
			copyAttributes: function(l, m) {
				...
				for (var o = 0; o < n.length; o++) {
					...
					else if (p.specified || c && p.nodeValue && q == 'value') {
						r = s.getAttribute(q);
		...
		function j(l) {
			var m = ['top', 'left', 'right', 'bottom'],
				n;
			if (l == 'border') n = ['color', 'style', 'width'];
			var o = [];
			for (var p = 0; p < m.length; p++) {
				if (n)
					for (var q = 0; q < n.length; q++) o.push([l, m[p], n[q]].join('-'));
			...
			function C(D, E, F) {
				...
				if (D._.isBlockLike && D.name != 'pre' && D.name != 'textarea') {
					var J = D.children.length,
						...
						L;
					if (K && K.type == 3)
						if (!(L = e.rtrim(K.value))) D.children.length = J - 1;
						else K.value = L;
				}
				E.add(D);
		...
		function l(s, t) {
			...
			if (this.current) v = this.current[D](false, C, A);
				...
				if (s) {
					v = x;
					if (v.type == 1)
						if (z > 0) v = v.getChild(z - 1);
						else v = A(v, true) === false ? null : v.getPreviousSourceNode(true, C, A);
				} else {
					v = w;
					if (v.type == 1)
						if (!(v = v.getChild(y))) v = A(w, true) === false ? null : w.getNextSourceNode(true, C, A);
				}
	...
	(function() {
		...
		function o() {
			var v = false,
				w = d.walker.whitespaces(),
				x = d.walker.bookmark(true),
				y = d.walker.bogus();
			return function(z) {
		...
		function q(v) {
			var w = d.walker.whitespaces(),
				x = d.walker.bookmark(1);
			return function(y) {
		...
		d.range.prototype = {
			...
			createBookmark2: function(v) {
				...
				return {
					start: w.getAddress(v),
					end: A ? null : x.getAddress(v),
					startOffset: y,
					endOffset: z,
					normalized: v,
					collapsed: A,
			...
			moveToBookmark: function(v) {
				var D = this;
				if (v.is2) {
					var w = D.document.getByAddress(v.start, v.normalized),
						x = v.startOffset,
						y = v.end && D.document.getByAddress(v.end, v.normalized),
						z = v.endOffset;
					D.setStart(w, x);
					if (y) D.setEnd(y, z);
					else D.collapse(true);
				} else {
					var A = v.serializable,
						B = A ? D.document.getById(v.startNode) : v.startNode,
						C = A ? D.document.getById(v.endNode) : v.endNode;
					D.setStartBefore(B);
					B.remove();
					if (C) {
						D.setEndBefore(C);
			...
			optimize: function() {
				...
				var v = x.startContainer,
					w = x.startOffset;
				if (v.type != 1)
					if (!w) x.setStartBefore(v);
					else if (w >= v.getLength()) x.setStartAfter(v);
				v = x.endContainer;
				w = x.endOffset;
				if (v.type != 1)
					if (!w) x.setEndBefore(v);
			...
			enlarge: function(v, w) {
				switch (v) {
					case 1:
						if (this.collapsed) return;
						var x = this.getCommonAncestor(),
							y = this.document.getBody(),
							z, A, B, C, D, E = false,
							F, G, H = this.startContainer,
							I = this.startOffset;
						if (H.type == 3) {
							if (I) {
							...
							if (H)
								if (!(C = H.getPrevious())) B = H.getParent();
						} else {
						...
						while (B || C) {
							...
							while (C) {
								...
								if (C.type == 8) {
									...
									if (/[^\s\ufeff]/.test(G)) C = null;
									F = /[\s\ufeff]$/.test(G);
								} else if ((C.$.offsetWidth > 0 || w && C.is('br')) && !C.data('cke-bookmark'))
									if (E && f.$removeEmpty[C.getName()]) {
										G = C.getText();
										if (/[^\s\ufeff]/.test(G)) C = null;
										else {
						...
						while (B || C) {
							...
							while (C) {
								...
								if (C.type == 3) {
									...
									if ((C.$.offsetWidth > 0 || w && C.is('br')) && !C.data('cke-bookmark'))
										if (E && f.$removeEmpty[C.getName()]) {
											G = C.getText();
											if (/[^\s\ufeff]/.test(G)) C = null;
											else {
					...
					case 3:
						...
						var O = new d.walker(N),
							P, Q, R = d.walker.blockBoundary(v == 3 ? {
								br: 1
							} : null),
							S = function(Y) {
								var Z = R(Y);
			...
			shrink: function(v, w) {
				if (!this.collapsed) {
					...
					if (z && z.type == 3)
						if (!B) x.setEndBefore(z);
						else if (B >= z.getLength()) x.setEndAfter(z);
					else {
						x.setEndAfter(z);
						E = 0;
					}
					var F = new d.walker(x),
						G = d.walker.bookmark();
			...
			setStartAt: function(v, w) {
				...
				switch (w) {
					case 1:
						...
						break;
					case 2:
						if (v.type == 3) x.setStart(v, v.getLength());
			...
			setEndAt: function(v, w) {
				...
				switch (w) {
					case 1:
						...
						break;
					case 2:
						if (v.type == 3) x.setEnd(v, v.getLength());
			...
			fixBlock: function(v, w) {
				var z = this;
				var x = z.createBookmark(),
					y = z.document.createElement(w);
				z.collapse(v);
				z.enlarge(2);
			...
			splitBlock: function(v) {
				var F = this;
				var w = new d.elementPath(F.startContainer),
					x = new d.elementPath(F.endContainer),
					y = w.blockLimit,
					z = x.blockLimit,
					A = w.block,
					B = x.block,
				...
				var D = A && F.checkStartOfBlock(),
					E = B && F.checkEndOfBlock();
				F.deleteContents();
				if (A && A.equals(B))
					if (E) {
			...
			checkStartOfBlock: function() {
				...
				var y = new d.elementPath(B.startContainer),
					z = B.clone();
				z.collapse(true);
			...
			checkEndOfBlock: function() {
				var B = this;
				var v = B.endContainer,
					w = B.endOffset;
				if (c && v.type == 3) {
					var x = e.rtrim(v.substring(w));
					if (t.test(x)) B.trim(1, 0);
				}
				var y = new d.elementPath(B.endContainer),
					z = B.clone();
				z.collapse(false);
			...
			checkReadOnly: (function() {
				function v(w, x) {
					while (w) {
						if (w.type == 1)
							if (w.getAttribute('contentEditable') == 'false' && !w.data('cke-editable')) return 0;
			...
			moveToElementEditablePosition: function(v, w) {
				...
				while (v) {
					if (v.type == 3) {
						if (w && this.checkEndOfBlock() && t.test(v.getText())) this.moveToPosition(v, 3);
				...
				if (p.styles || p.classes) {
					...
					if (p.styles) s.push({
						...
						updateStyle: function(t, u) {
							var v = this.getValue(),
								w = o.document.createElement('span');
				...
				var u = e.addFunction(t),
					v = e.addFunction(function(x, y) {
						...
						switch (y.getKeystroke()) {
							case B ? 39:
								37: case 9: A = a.document.getById(z + (x + 1));
								if (!A) A = a.document.getById(z + '0');
								A.focus();
								return false;
							case B ? 37:
								39: case 2228224 + 9: A = a.document.getById(z + (x - 1));
	...
	(function() {
		...
		function s(U, V) {
			return function(W) {
				q(W, !U);
				if (r(W, !U, V))
					if (U || c) W.add(new a.htmlParser.text('\xa0'));
					else W.add(new a.htmlParser.element('br', {}));
			};
		};
		var t = f,
			u = ['caption', 'colgroup', 'col', 'thead', 'tfoot', 'tbody'],
			v = e.extend({}, t.$block, t.$listItem, t.$tableContent);
		for (var w in v) {
		...
		var z = {
			...
			elements: {
				...
				embed: function(U) {
					var V = U.parent;
					if (V && V.name == 'object') {
						var W = V.attributes.width,
							X = V.attributes.height;
		...
		var C = /<(a|area|img|input|source)\b([^>]*)>/gi,
			D = /\b(on\w+|href|src|name)\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|(?:[^ "'>]+))/gi,
			E = /(?:<style(?=[ >])[^>]*>[\s\S]*<\/style>)|(?:<(:?link|meta|base)[^>]*>)/gi,
		...
		function T(U, V) {
			var W = [],
				X = V.config.protectedSource,
				Y = V._.dataStore || (V._.dataStore = {
					id: 1
				}),
				Z = /<\!--\{cke_temp(comment)?\}(\d*?)-->/g,
				aa = [/<script[\s\S]*?<\/script>/gi, /<noscript[\s\S]*?<\/noscript>/gi].concat(X);
		...
		a.htmlDataProcessor.prototype = {
			toHtml: function(U, V) {
				...
				U = L(U);
				U = R(U);
				var X = a.htmlParser.fragment.fromHtml(U, V),
			...
			toDataFormat: function(U, V) {
				var W = this.writer,
					X = a.htmlParser.fragment.fromHtml(U, V);
				W.reset();
	...
	j.add('link', {
		...
		afterInit: function(m) {
			...
			if (q) q.push(function(r, s) {
				if (s == 'a')
					if (j.link.tryRestoreFakeAnchor(m, r) || r.getAttribute('name') && (!r.getAttribute('href') || !r.getChildCount())) return 'anchor';
			});
		}
	});
	j.link = {
	...
	(function() {
		...
		j.list = {
			...
			arrayToList: function(N, O, P, Q, R) {
				...
				var S, T = N[P].parent.getDocument(),
					U = new d.documentFragment(T),
					V = null,
					W = P,
					X = Math.max(N[P].indent, 0),
					Y = null,
					Z, aa, ab = Q == 1 ? 'p' : 'div';
				while (1) {
					var ac = N[W],
					...
					if (ac.indent == X) {
						...
						var ae = N[W - 1].element.getDirection(1),
							af = j.list.arrayToList(N, null, W, Q, ae != Z ? Z : null);
						...
						var ag = ad.getDirection(1) != Z,
							ah = ac.element,
							ai = ah.getAttribute('class'),
							aj = ah.getAttribute('style'),
							ak = Y.type == 11 && (Q != 2 || ag || aj || ai),
							al, am = ac.contents.length;
						for (S = 0; S < am; S++) {
							al = ac.contents[S];
		...
		function v(N, O, P, Q) {
			var R = j.list.listToArray(O.root, P),
				S = [];
			...
			var aa = j.list.arrayToList(R, P, null, N.config.enterMode),
				ab, ac = aa.listNode.getChildCount();
		...
		function x(N, O, P) {
			...
			var W = N.config.useComputedState,
				X, Y;
			W = W === undefined || W;
			...
			var ac = S[S.length - 1].getNext(),
				ad = R.createElement(this.type);
			P.push(ad);
			var ae, af;
			while (S.length) {
				ae = S.shift();
		...
		function y(N, O, P) {
			var Q = j.list.listToArray(O.root, P),
				R = [];
			...
			for (S = U + 1; S < Q.length; S++) {
				if (Q[S].indent > Q[S - 1].indent + 1) {
					var W = Q[S - 1].indent + 1 - Q[S].indent,
						X = Q[S].indent;
			...
			var Y = j.list.arrayToList(Q, P, null, N.config.enterMode, O.root.getAttribute('dir')),
				Z = Y.listNode,
				aa, ab;
		...
		z.prototype = {
			exec: function(N) {
				...
				while (W.length > 0) {
					an = W.shift();
					if (aq.state == 2) {
						if (m[an.root.getName()]) v.call(aq, N, an, X, ap);
		...
		function L(N, O, P) {
			...
			O.trim(false, true);
			var R = O.createBookmark(),
				S = new d.elementPath(O.startContainer),
				T = S.block,
				U = S.lastElement.getAscendant('li', 1) || T,
				V = new d.elementPath(P.startContainer),
				W = V.contains(f.$listItem),
				X = V.contains(f.$list),
				Y;
			if (T) {
				var Z = T.getBogus();
		...
		j.add('list', {
			init: function(N) {
				var O = N.addCommand('numberedlist', new z('numberedlist', 'ol')),
					P = N.addCommand('bulletedlist', new z('bulletedlist', 'ul'));
				...
				N.on('key', function(Q) {
					...
					if (N.mode == 'wysiwyg' && R in {
						...
						var U = new d.elementPath(T.startContainer),
							V = R == 8,
							W = N.document.getBody(),
							X = new d.walker(T.clone());
						...
						if (v) {
							...
							while (w) {
								...
								if (!(w.getName() == 'img' && w.data('cke-realelement')) && p(m, w))
									if (n.test(w.getName())) w.remove(1);
									else {
	...
	(function() {
		...
		j.add('scayt', {
			...
			beforeInit: function(t) {
				...
				u = u.split('|');
				if (u && u.length)
					for (var w = 0; w < u.length; w++) v += 'scayt_' + u[w] + (u.length != parseInt(w, 10) + 1 ? ',' : '');
	...
	j.add('sourcearea', {
		...
		init: function(m) {
			...
			m.on('readOnly', function() {
				if (m.mode == 'source')
					if (m.readOnly) m.textarea.setAttribute('readOnly', 'readonly');
	...
	(function() {
		...
		function t(E) {
			var F = this.getSelection(),
				G = F.getStartElement().hasAscendant('pre', true) ? 2 : this.config.enterMode,
				H = G == 2,
				I = e.htmlEncode(E.replace(/\r\n|\r/g, '\n'));
			...
			if (b.gecko || b.webkit) {
				var K = new d.elementPath(F.getStartElement()),
					L = [];
		...
		function u(E) {
			...
			if (M) {
				...
				if (I) {
					var R = M.getNext(p),
						S = R && R.type == 1 && R.getName();
					if (S && f.$block[S]) {
						if (f[S]['#']) K.moveToElementEditStart(R);
		...
		var w = d.walker.whitespaces(true),
			x = d.walker.bookmark(false, true);
		...
		function C(E) {
			var F = E.window,
				G = E.document,
				H = E.document.getBody(),
				I = H.getFirst(),
				J = H.getChildren().count();
			if (!J || J == 1 && I.type == 1 && I.hasAttribute('_moz_editor_bogus_node')) {
				v(E);
				var K = E.element.getDocument(),
					L = K.getDocumentElement(),
					M = L.$.scrollTop,
					N = L.$.scrollLeft,
		...
		function D(E) {
			var F = E.editor,
				...
				I = E.data.selection,
				J = I.getRanges()[0],
				K = F.document.getBody(),
				L = F.config.enterMode;
			if (b.gecko) {
				var M = G.block || G.blockLimit,
					N = M && M.getLast(y);
		...
		j.add('wysiwygarea', {
			requires: ['editingblock'],
			init: function(E) {
				var F = E.config.enterMode != 2 && E.config.autoParagraph !== false ? E.config.enterMode == 3 ? 'div' : 'p' : false,
					G = E.lang.editorTitle.replace('%1', E.name),
					H = E.lang.editorHelp;
				if (c) G += ', ' + H;
				var I = a.document.getWindow(),
					J;
				E.on('editingBlockReady', function() {
					var M, N, O, P, Q, R, S, T = b.isCustomDomain(),
						U = function(X) {
							if (N) N.remove();
					...
					function W(X) {
						...
						var Y = X.document,
							Z = Y.body,
							aa = Y.getElementById('cke_actscrpt');
						...
						E.on('key', function(ag) {
							...
							if (ah in {
								...
								var ai = E.getSelection(),
									aj = ai.getSelectedElement(),
									ak = ai.getRanges()[0],
									al = new d.elementPath(ak.startContainer),
									am, an, ao, ap = ah == 8;
								if (aj) {
									E.fire('saveSnapshot');
									...
									E.fire('saveSnapshot');
									ag.cancel();
								} else if (ak.collapsed)
									if ((am = al.block) && ak[ap ? 'checkStartOfBlock' : 'checkEndOfBlock']() && (ao = am[ap ? 'getPrevious' : 'getNext'](n)) && ao.is('table')) {
										E.fire('saveSnapshot');
					...
					E.addMode('wysiwyg', {
						...
						loadData: function(X) {
							...
							var Y = E.config,
								Z = Y.fullPage,
								aa = Y.docType,
								ab = '<style type="text/css" data-cke-temp="1">' + E._.styles.join('\n') + '</style>';
						...
						getData: function() {
							var X = E.config,
								Y = X.fullPage,
								Z = Y && E.docType,
								aa = Y && E.xmlDeclaration,
								ab = N.getFrameDocument(),
								ac = Y ? ab.getDocumentElement().getOuterHtml() : ab.getBody().getHtml();
	...
	(function() {
		...
		function o() {
			var V = this;
			var R = V._.currentTabId,
				S = V._.tabIdList.length,
				T = e.indexOf(V._.tabIdList, R) + S;
		...
		function q(R, S) {
			...
			for (var U = 0, V = T.length; U < V; U++) {
				var W = new h(T[U]);
				if (W.getAttribute('type').toLowerCase() == 'text')
					if (S) {
		...
		a.dialog = function(R, S) {
			var T = a.dialog._.dialogDefinitions[S],
				U = e.clone(v),
				V = R.config.dialog_buttonsOrder || 'OS',
				W = R.lang.dir,
				X = {},
				Y, Z, aa;
			...
			a.event.call(this);
			this.definition = T = a.fire('dialogDefinition', {
				name: S,
				definition: T
			}, R).definition;
			...
			if (R._.removeDialogTabs && (X = R._.removeDialogTabs[S]))
				for (Y = 0; Y < X.length; Y++) T.removeContents(X[Y]);
			...
			this.on('show', function() {
				...
				if (R.config.dialog_startupFocusTab && ah._.pageCount > 1) {
					...
					if (av._.editor.mode == 'wysiwyg' && c) {
						var as = R.document.$.selection,
							at = as.createRange();
						if (at)
							if (at.parentElement && at.parentElement().ownerDocument == R.document.$ || at.item && at.item(0).ownerDocument == R.document.$) {
		...
		a.dialog.prototype = {
			...
			move: function(R, S, T) {
				var ab = this;
				var U = ab._.element.getFirst(),
					V = ab._.editor.lang.dir == 'rtl',
					W = U.getComputedStyle('position') == 'fixed';
				...
				if (V) {
					var Y = ab.getSize(),
						Z = a.document.getWindow().getViewPaneSize();
					R = Z.width - Y.width - R;
				}
				var aa = {
					top: (S > 0 ? S : 0) + 'px'
			...
			layout: function() {
				var X = this;
				var R = X.parts.dialog,
					S = X.getSize(),
					T = a.document.getWindow(),
					U = T.getViewPaneSize(),
					V = (U.width - S.width) / 2,
					W = (U.height - S.height) / 2;
				if (!b.ie6Compat)
					if (S.height + (W > 0 ? W : 0) > U.height || S.width + (V > 0 ? V : 0) > U.width) R.setStyle('position', 'absolute');
					else R.setStyle('position', 'fixed');
				X.move(X._.moved ? X._.position.x : V, X._.moved ? X._.position.y : W);
			},
			foreach: function(R) {
				var U = this;
				for (var S in U._.contents)
					for (var T in U._.contents[S]) R.call(U, U._.contents[S][T]);
				return U;
			},
			reset: (function() {
				var R = function(S) {
			...
			addPage: function(R) {
				var ad = this;
				var S = [],
					T = R.label ? ' title="' + e.htmlEncode(R.label) + '"' : '',
					U = R.elements,
					V = a.dialog._.uiElementBuilders.vbox.build(ad, {
						type: 'vbox',
					...
					W = h.createFromHtml(S.join(''));
				W.setAttribute('role', 'tabpanel');
				var X = b,
					Y = 'cke_' + R.id + '_' + e.getNextNumber(),
					Z = h.createFromHtml(['<a class="cke_dialog_tab"', ad._.pageCount > 0 ? ' cke_last' : 'cke_first', T, !!R.hidden ? ' style="display:none"' : '', ' id="', Y, '"', X.gecko && X.version >= 10900 && !X.hc ? '' : ' href="javascript:void(0)"', ' tabIndex="-1"', ' hidefocus="true"', ' role="tab">', R.label, '</a>'].join(''));
		...
		function B(R) {
			var S = null,
				T = null,
				U = R.getElement().getFirst(),
				V = R.getParentEditor(),
				W = V.config.dialog_magnetDistance,
				X = V.skin.margins || [0, 0, 0, 0];
			if (typeof W == 'undefined') W = 20;

			function Y(aa) {
				var ab = R.getSize(),
					ac = a.document.getWindow().getViewPaneSize(),
					ad = aa.data.$.screenX,
					ae = aa.data.$.screenY,
					af = ad - S.x,
					ag = ae - S.y,
		...
		function C(R) {
			var S = R.definition,
				T = S.resizable;
			if (T == 0) return;
			var U = R.getParentEditor(),
				V, W, X, Y, Z, aa, ab = e.addFunction(function(ae) {
					Z = R.getSize();
					var af = R.parts.contents,
						ag = af.$.getElementsByTagName('iframe').length;
					if (ag) {
			...
			function ac(ae) {
				var af = U.lang.dir == 'rtl',
					ag = (ae.data.$.screenX - Y.x) * (af ? -1 : 1),
					ah = ae.data.$.screenY - Y.y,
					ai = Z.width,
					aj = Z.height,
					ak = ai + ag * (R._.moved ? 1 : 2),
					al = aj + ah * (R._.moved ? 1 : 2),
					am = R._.element.getFirst(),
					an = af && am.getComputedStyle('right'),
					ao = R.getPosition();
		...
		function H(R) {
			var S = a.document.getWindow(),
				T = R.config,
				U = T.dialog_backgroundCoverColor || 'white',
				V = T.dialog_backgroundCoverOpacity,
				W = T.baseFloatZIndex,
				X = e.genKey(U, V, W),
				Y = E[X];
			if (!Y) {
				...
				if (b.ie6Compat) {
					var aa = b.isCustomDomain(),
						ab = "<html><body style=\\'background-color:" + U + ";\\'></body></html>";
			...
			var ac = function() {
				...
				ad = function() {
					var af = S.getScrollPosition(),
						ag = a.dialog._.currentTop;
					Y.setStyles({
						left: af.x + 'px',
						top: af.y + 'px'
					});
		...
		var K = {},
			L = function(R) {
				var S = R.data.$.ctrlKey || R.data.$.metaKey,
					T = R.data.$.altKey,
					U = R.data.$.shiftKey,
					V = String.fromCharCode(R.data.$.keyCode),
					W = K[(S ? 'CTRL+' : '') + (T ? 'ALT+' : '') + (U ? 'SHIFT+' : '') + V];
			...
			N = function(R, S, T, U, V) {
				...
				W.push({
					...
					dialog: S,
					key: T,
					keyup: V || R.accessKeyUp,
					keydown: U || R.accessKeyDown
				});
			},
			O = function(R) {
				for (var S in K) {
		...
		(function() {
			k.dialog = {
				uiElement: function(R, S, T, U, V, W, X) {
					if (arguments.length < 4) return;
					var Y = (U.call ? U(S) : U) || 'div',
						Z = ['<', Y, ' '],
						aa = (V && V.call ? V(S) : V) || {},
						ab = (W && W.call ? W(S) : W) || {},
						ac = (X && X.call ? X.call(this, R, S) : X) || '',
						ad = this.domId = ab.id || e.getNextId() + '_uiElement',
						ae = this.id = S.id,
						af;
					ab.id = ad;
					var ag = {};
				...
				hbox: function(R, S, T, U, V) {
					...
					var W = this._.children = S,
						...
						aa, ab = function() {
							...
							for (aa = 0; aa < T.length; aa++) {
								var ae = 'cke_dialog_ui_hbox_child',
									af = [];
					...
					k.dialog.uiElement.call(this, R, V || {
						type: 'hbox'
					}, U, 'table', Z, ac, ab);
				},
		...
		(function() {
			var R = /^([a]|[^a])+$/,
				...
				T = /^\d*(?:\.\d+)?$/,
				U = /^(((\d*(\.\d+))|(\d*))(px|\%)?)?$/,
				V = /^(((\d*(\.\d+))|(\d*))(px|em|ex|in|cm|mm|pt|pc|\%)?)?$/i,
				W = /^(\s*[\w-]+\s*:\s*[^:;]+(?:;|$))*$/;
			a.VALIDATE_OR = 1;
			a.VALIDATE_AND = 2;
			a.dialog.validate = {
				functions: function() {
					var X = arguments;
					return function() {
						var Y = this && this.getValue ? this.getValue() : X[0],
							Z = undefined,
							aa = 2,
							ab = [],
							ac;
						for (ac = 0; ac < X.length; ac++) {
							if (typeof X[ac] == 'function') ab.push(X[ac]);
							else break;
						}
		...
		e.extend(a.editor.prototype, {
			openDialog: function(R, S) {
				if (this.mode == 'wysiwyg' && c) {
					var T = this.getSelection();
					T && T.lock();
				}
				var U = a.dialog._.dialogDefinitions[R],
					V = this.skin.dialog;
				if (a.dialog._.currentTop === null) H(this);
				if (typeof U == 'function' && V._isLoaded) {
					var W = this._.storedDialogs || (this._.storedDialogs = {}),
						X = W[R] || (W[R] = new a.dialog(this, R));
					S && S.call(X, X);
					X.show();
					return X;
				} else if (U == 'failed') {
				...
				function Z(ab) {
					var ac = a.dialog._.dialogDefinitions[R],
						ad = Y.skin.dialog;
	...
	(function() {
		...
		function w(T) {
			...
			var W = this,
				...
				Y = X.attributes;
			if (Y)
				for (var Z in Y) V.removeAttribute(Z, Y[Z]);
			if (X.styles)
	...
	(function() {
		function m() {
			...
			try {
				...
				var E = D.getStartElement(),
					F = new d.elementPath(E);
				if (!F.compare(G._.selectionPreviousPath)) {
					G._.selectionPreviousPath = F;
					G.fire('selectionChange', {
						selection: D,
						path: F,
						element: E
					});
		...
		function r(D) {
			...
			var F = D.startContainer,
				G = D.getPreviousNode(A, null, F),
				H = D.getNextNode(A, null, F);
		...
		function w(D) {
			...
			if (E) {
				var F, G = D.getSelection().getNative(),
					H = G && G.type != 'None' && G.getRangeAt(0);
				if (E.getLength() > 1 && H && H.intersectsNode(E.$)) {
					F = [G.anchorOffset, G.focusOffset];
					var I = G.anchorNode == E.$ && G.anchorOffset > 0,
						J = G.focusNode == E.$ && G.focusOffset > 0;
					I && F[0]--;
					J && F[1]--;
					x(G) && F.unshift(F.pop());
				}
						...
						function U(V) {
							if (N) {
								var W = D.document,
									...
									Y = X && X.getNative();
								if (V && Y && Y.type == 'None')
									if (!W.$.queryCommandEnabled('InsertImage')) {
										e.setTimeout(U, 50, this, true);
										return;
									} var Z;
								if (Y && Y.type && Y.type != 'Control' && (Z = Y.createRange()) && (Z = Z.parentElement()) && (Z = Z.nodeName) && Z.toLowerCase() in {
										input: 1,
										textarea: 1
									}) return;
								try {
		...
		d.selection.prototype = {
				...
				var D = c ? (function() {
					...
					var F = function(G, H) {
						...
						while (O <= P) {
							...
							else if (b.ie9Compat && L.tagName == 'BR') {
								var U = J.defaultView.getSelection();
								return {
									container: U[H ? 'anchorNode' : 'focusNode'],
									offset: U[H ? 'anchorOffset' : 'focusOffset']
								};
							} else return {
								container: I,
								offset: E(L)
						...
						if (Q == -1 || Q == K.length - 1 && R < 0) {
							...
							return {
								container: T,
								offset: R > 0 ? -S : T.nodeValue.length + S
							};
						}
					};
					return function() {
					...
					if (E) {
						...
						for (var H = 0; H < G.length; H++) {
							...
							var L = I.startContainer,
								...
								P = I.clone();
							if (L && L.type == 3)
								if (N >= L.getLength()) P.setStartAfter(L);
								else P.setStartBefore(L);
							if (M && M.type == 3)
								if (!O) P.setEndBefore(M);
			...
			getSelectedElement: function() {
				...
				var E = this,
					F = e.tryThese(function() {
						return E.getNative().createRange().item(0);
					}, function() {
						var G, H, I = E.getRanges()[0],
							J = I.getCommonAncestor(1, 1),
							K = {
								table: 1,
								ul: 1,
								ol: 1,
						...
						if (G) {
							...
							var N = e.extend(K, f.$listItem, f.$tableContent),
								O = new d.walker(M),
								P = function(Q, R) {
									return function(S, T) {
			...
			getCommonAncestor: function() {
				var D = this.getRanges(),
					E = D[0].startContainer,
					F = D[D.length - 1].endContainer;
		...
		var z = d.walker.whitespaces(true),
			A = d.walker.invisible(1),
			B = /\ufeff|\u00a0/,
			C = {
				table: 1,
				tbody: 1,
			...
			file: function(u, v, w) {
				...
				var y = function() {
					...
					var z = b.isCustomDomain(),
						A = ['<iframe frameborder="0" allowtransparency="0" class="cke_dialog_ui_input_file" role="presentation" id="', x.frameId, '" title="', v.label, '" src="javascript:void('];
