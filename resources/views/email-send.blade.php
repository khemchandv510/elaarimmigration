...
@section('content')
    ...
    @endif
            <form method="post" action="{{ route('send-live-email') }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="exampleFormControlSelect1">Select Group</label>
                <select name="groupid" class="form-control" id="exampleFormControlSelect1" required>
                    <option value="">Select</option>
                       
                    @foreach($group as $date)
                    <option value="{{$date->unique_id}}">{{$date->name}}</option>
                    @endforeach
            ...
            <div class="form-group">
                <label>Limit </label>
                <input type="number" name="emailLimit" class="form-control" required />
            </div>
